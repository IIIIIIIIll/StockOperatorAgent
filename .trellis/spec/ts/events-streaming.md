...
- **SSRF 防线(C2)**:`X-LLM-Base` 头/body.base 是浏览器端用户配置(多提供商
  ...
  **IPv4-compatible ::/96 教训(08-29 上线前复审)**:RFC 4291 弃用的
  `::/96`(前 96 位全零、末 32 位内嵌 IPv4,如 `::7f00:1` = 127.0.0.1、
  `::192.168.1.1` 点分尾形)与 mapped 同族——按前缀比较的分支同样会把它
  判公网。判定规则:前 6 组全 `0000` 且非 mappedStd/mappedLoose 时提取
  末两组内嵌 32 位(hex 与点分尾形都要处理)重放 IPv4 黑名单,递归单源;
  不可整段封 ::/96(`::808:808` = 8.8.8.8 公网,防 over-block)。现代栈
  不路由 ::/96(不可直接利用),属纵深防御。
