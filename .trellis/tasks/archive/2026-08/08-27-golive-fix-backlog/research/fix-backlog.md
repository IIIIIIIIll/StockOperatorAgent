...
> Correction note (2026-08-27, applied to this parent copy only; the archived
> audit original keeps its provenance): F52 evidence `app/app.json:7` → `app/app.json:5`
> (version field line verified at HEAD); S2 evidence `metro.config.js:121-140` →
> `app/metro.config.js:78-103` (file is 105 lines; middleware at ~:78-103).
> Tables hold 80 rows (78 fixable + S1/H3 informational); 11 adjustments
> enumerated in Provenance (1 fix-adjusted + 5 severity-adjusted + 3 deduped +
> 2 new).
...
| S2 | low | no Origin/CSRF check on proxy endpoints; GETs triggerable cross-site (responses unreadable; PNA/Host mitigate) | app/server.mjs:99-124; app/metro.config.js:78-103 | Origin allowlist (own origin / null) or per-session token | [SA] med→low |
...
| F52 | nit→minor | app/package.json Expo template identity + version 57.0.13; app.json version 1.0.0 → tagged v0.1.3 AAB would carry versionName 1.0.0 [DUP←R1] | app/package.json:2-5; app/app.json:5 | personalize name/desc/license; align version with release series (or document web/RN channel) | [C] |
