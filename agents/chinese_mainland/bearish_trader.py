...
from core.llms.prompt import bearish_trader_message, bearish_revise_message


class BearishTrader(AgentNode):
    ...
    def __init__(self, llm: BaseChatModel, config: RunnableConfig, progress_updater = None, tools: list | None = None):
        ...
        super().__init__(llm, config, progress_updater, tools,
                         role_message=bearish_trader_message)
        self.revise_llm = self.build_chain(bearish_revise_message)
    ...
    def bearish_trader(self, state: State):
        ...
        return self.complete_with_tools(
            bearish_trader_query, "bearish_opinions",
            start_msg="开始空方观点生成。。。",
            done_msg="空方观点生成完成。。。",
            log_label="Bearish Trader",
        )
    ...
    def bearish_revise(self, state: State):
        ...
        return self.complete_with_tools(
            bearish_revise_query, "bearish_opinions",
            chain=self.revise_llm, max_tool_rounds=3,
            start_msg="开始空方观点修订。。。",
            done_msg="空方观点修订完成。。。",
            log_label="Bearish Trader Revise",
        )
