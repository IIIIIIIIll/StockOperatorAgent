...
class AKShareSource():
    ...
    def fetch_stock_history_days(self, ticker, look_back_cycles=100):
        stock_hist = ak.stock_zh_a_hist(symbol=ticker, period="daily", start_date=(datetime.date.today() - timedelta(days=look_back_cycles + 1)).strftime('%Y%m%d'),
                                        end_date=datetime.date.today().strftime('%Y%m%d'), adjust="qfq")
        return stock_hist

    def fetch_stock_history_weeks(self, ticker, look_back_cycles=100):
        stock_hist = ak.stock_zh_a_hist(symbol=ticker, period="weekly", start_date=(datetime.date.today() - timedelta(days=look_back_cycles + 1)).strftime('%Y%m%d'),
                                        end_date=datetime.date.today().strftime('%Y%m%d'), adjust="qfq")
        return stock_hist

    def fetch_stock_history_months(self, ticker, look_back_cycles=100):
        stock_hist = ak.stock_zh_a_hist(symbol=ticker, period="monthly", start_date=(datetime.date.today() - timedelta(days=look_back_cycles + 1)).strftime('%Y%m%d'),
                                        end_date=datetime.date.today().strftime('%Y%m%d'), adjust="qfq")
        return stock_hist
