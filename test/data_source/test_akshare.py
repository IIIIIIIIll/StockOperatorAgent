...
class TestAKShare():
    ...
    def test_get_stock_day_hist(self):
        akshare_source = AKShareSource()
    ...
    def test_get_stock_week_hist(self):
        akshare_source = AKShareSource()
        stocks_hist = akshare_source.fetch_stock_history_weeks('601188')
        assert stocks_hist is not None
        stocks_hist_dict = stocks_hist.to_dict(orient='records')
        for row in stocks_hist_dict:
            stock_data = ChinaStockData(*list(row.values()))
    ...
    def test_get_stock_month_hist(self):
        akshare_source = AKShareSource()
        stocks_hist = akshare_source.fetch_stock_history_months('601188')
        assert stocks_hist is not None
        stocks_hist_dict = stocks_hist.to_dict(orient='records')
        for row in stocks_hist_dict:
            stock_data = ChinaStockData(*list(row.values()))
