"""设置面板（08-08-billions-switches-ui，Step 4；08-09-llm-provider-
agnostic 改造）。

e2e 交互用例（模块级函数风格，同目录既有约定）：
- 用例 A：侧边栏「设置」展开 → 4 分区可见、4 个 password 框存在且
...
- 用例 C：保存路径隔离——改模型文本 + 填新密钥 → 保存 → st.success
  出现；tmp .env（ENV_FILE_PATH，conftest e2e_env_file fixture）含新值；
...
- 模型/endpoint 是 st.text_input（08-09 起不再有模型 selectbox），
  aria-label = 标签文本，fill 可用；number_input 是原生
  input[type="number"]（aria-label = 标签文本），fill 可用；
...
def _fill_model(page, model: str) -> None:
    """在「LLM 模型」文本输入框填模型名（08-09 起自由文本，无 selectbox）。

    文本输入框重渲染可能吞输入（rerun 期间）——重试，以 input_value 变
    为成功判据（幂等）。
    """
    field = page.get_by_label("LLM 模型")
    deadline = time.time() + 20
    while time.time() < deadline:
        field.fill(model)
        try:
            if field.input_value() == model:
                return
        except Exception:
            pass
        time.sleep(0.3)
    raise AssertionError(f"model {model!r} never filled")
...
def test_settings_panel_sections_and_password_hidden(page):
    """用例 A：设置面板 4 分区 + 4 个 password 框不回显明文。

    dummy 三键注入（conftest）→ 主密钥框 placeholder 为「已配置（留空
    表示不修改）」；所有 password 框值为空（面板不留值——不明文回显，
    AC3）；widget 计数：text 3（模型/Base URL/项目名，无 selectbox）/
    toggle 9 / number_input 3。
    """
    ...
    # 4 个 password 框：存在、type=password、值为空（不回显明文）
    pws = page.locator('[data-testid="stSidebar"] input[type="password"]')
    assert pws.count() == 4, f"expected 4 password inputs, got {pws.count()}"
    for pw in pws.all():
    ...
    # dummy LLM_API_KEY + BILLIONS_API_KEY（无 TDX）。
    assert page.get_by_label("LLM API Key").get_attribute("placeholder") == "已配置（留空表示不修改）"
    ...
    # 模型/endpoint 为自由文本输入（08-09-llm-provider-agnostic：不再枚举
    # 模型下拉；LLM Base URL 是一等配置）
    assert page.get_by_label("LLM 模型").input_value() == "deepseek-v4-flash"
    assert page.get_by_label("LLM Base URL").input_value() == "https://api.deepseek.com"
    # widget 计数（结构断言）：text 3（模型/Base URL/项目名，无 selectbox）
    # + toggle 9（LangSmith 追踪 + 会话区 8）+ number_input 3（亿信上限）
    assert page.locator('[data-testid="stSidebar"] [data-testid="stSelectbox"]').count() == 0
    assert page.locator('[data-testid="stSidebar"] input[type="text"]').count() == 3
...
def test_save_writes_tmp_env_not_real(page, e2e_env_file):
    ...
    改 LLM_MODEL 文本 → gpt-4o + 填 LLM API Key dummy 新值 → 保存 →
    st.success；断言 ENV_FILE_PATH 指向的 tmp .env 含新值（服务器侧
    update_env_file 经 env_file_path() 落该路径）；断言真实 .env
    （REPO_ROOT/.env）内容逐字节不变（e2e 不碰开发者配置）。
    """
    real_before = _REAL_ENV.read_text(encoding="utf-8") if _REAL_ENV.exists() else None
    _expand_settings(page)
    # 模型自由文本输入（08-09 起无 selectbox；Base URL 文本框恒有初始值，
    # 保存时恒收集——非密钥字段）
    _fill_model(page, "gpt-4o")
    # password 框填新值（dummy 值，不回显——只测写入路径）
    page.get_by_label("LLM API Key").fill("dummy-new-key")
    ...
    assert "LLM_API_KEY=dummy-new-key" in content
    assert "LLM_BASE_URL=https://api.deepseek.com" in content
    # 真实 .env 零修改
