...
async function main() {
  ...
  process.on('message', (msg) => {
    ...
    if (msg.type === 'op') {
      // Dispatch by op name with args spread; always ack after settle, error
      // message first on failure — the process must never crash on a bad op.
      const run = (async () => {
        const fn = store[msg.op];
        if (typeof fn !== 'function') throw new Error(`unknown store op: ${msg.op}`);
        await fn.apply(store, Array.isArray(msg.args) ? msg.args : []);
      })();
      run.then(
